...
habitTracker::habitTracker(QString habitTitle, QWidget *parent)
    : QWidget(parent), data(cols, QVector<int>(rows, 0)), startDate(QDate::currentDate().addDays(-364)) {
    header = habitTitle;
...
void habitTracker::drawMonthLabels(QPainter &painter) {
    ...
    for (auto it = monthPositions.constBegin(); it != monthPositions.constEnd(); ++it) {
...
void habitTracker::drawWeekdayLabels(QPainter &painter) {
    ...
    for (auto it = weekdayLabels.constBegin(); it != weekdayLabels.constEnd(); ++it) {
